package com.cb.probity.service.impl;

import java.util.List;
import java.util.Date;
import com.cb.common.utils.DateUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import com.cb.common.utils.StringUtils;
import com.cb.common.utils.uuid.IdUtils;
import com.cb.common.utils.SecurityUtils;
import com.cb.probity.mapper.BizProbityInsuranceMapper;
import com.cb.probity.domain.BizProbityInsurance;
import com.cb.probity.service.IBizProbityInsuranceService;

/**
 * 廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况Service业务层处理
 * 
 * @author ruoyi
 * @date 2025-03-18
 */
@Service
public class BizProbityInsuranceServiceImpl implements IBizProbityInsuranceService 
{
    @Autowired
    private BizProbityInsuranceMapper bizProbityInsuranceMapper;

    /**
     * 查询廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     * 
     * @param id 廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况ID
     * @return 廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     */
    @Override
    public BizProbityInsurance selectBizProbityInsuranceById(String id)
    {
        return bizProbityInsuranceMapper.selectBizProbityInsuranceById(id);
    }

    /**
     * 查询廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况列表
     * 
     * @param bizProbityInsurance 廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     * @return 廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     */
    @Override
    public List<BizProbityInsurance> selectBizProbityInsuranceList(BizProbityInsurance bizProbityInsurance)
    {
        return bizProbityInsuranceMapper.selectBizProbityInsuranceList(bizProbityInsurance);
    }

    /**
     * 新增廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     * 
     * @param bizProbityInsurance 廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     * @return 结果
     */
    @Override
    public int insertBizProbityInsurance(BizProbityInsurance bizProbityInsurance)
    {
        if(StringUtils.isBlank(bizProbityInsurance.getId())) {
            bizProbityInsurance.setId(IdUtils.randomUUID());
        }
        try{
            bizProbityInsurance.setCreateBy(SecurityUtils.getUsername());
        } catch (Exception e){}
        bizProbityInsurance.setCreateTime(DateUtils.getNowDate());
        return bizProbityInsuranceMapper.insertBizProbityInsurance(bizProbityInsurance);
    }


    /**
     * 批量新增廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     *
     * @param entities
     * @return 结果
     */
    @Override
    public int insertBatch(List<BizProbityInsurance> entities) {
        if(null == entities || entities.isEmpty()){
            return 0;
        }
        String userName = null;
        try{
            userName = SecurityUtils.getUsername();
        } catch (Exception e){}
        Date nowDate = DateUtils.getNowDate();
        String finalUserName = userName;
        entities.parallelStream().forEach(item -> {
            item.setId(IdUtils.randomUUID());
            item.setCreateBy(finalUserName);
            item.setCreateTime(nowDate);
        });
        // 定义每批次的大小
        int batchSize = 500;
        int totalInserted = 0;
        int num = entities.size() / batchSize + (entities.size() % batchSize == 0 ? 0 : 1);
        for (int i = 0; i < num; i++) {
            int start = i * batchSize;
            int end = Math.min(start + batchSize, entities.size());
            totalInserted +=  bizProbityInsuranceMapper.insertBatch(entities.subList(start, end));
        }
        return totalInserted;
    }


    /**
     * 修改廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     * 
     * @param bizProbityInsurance 廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     * @return 结果
     */
    @Override
    public int updateBizProbityInsurance(BizProbityInsurance bizProbityInsurance)
    {
        try{
            bizProbityInsurance.setUpdateBy(SecurityUtils.getUsername());
        } catch (Exception e){}
        bizProbityInsurance.setUpdateTime(DateUtils.getNowDate());
        return bizProbityInsuranceMapper.updateBizProbityInsurance(bizProbityInsurance);
    }

    /**
     * 批量删除廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况
     * 
     * @param ids 需要删除的廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况ID
     * @return 结果
     */
    @Override
    public int deleteBizProbityInsuranceByIds(String[] ids)
    {
        return bizProbityInsuranceMapper.deleteBizProbityInsuranceByIds(ids);
    }

    /**
     * 删除廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况信息
     * 
     * @param id 廉政档案-15.本人、配偶、共同生活的子女投资或者以其他方式持有投资型保险的情况ID
     * @return 结果
     */
    @Override
    public int deleteBizProbityInsuranceById(String id)
    {
        return bizProbityInsuranceMapper.deleteBizProbityInsuranceById(id);
    }
}
