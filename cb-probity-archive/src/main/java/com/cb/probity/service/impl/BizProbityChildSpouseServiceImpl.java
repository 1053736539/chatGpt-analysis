package com.cb.probity.service.impl;

import java.util.List;
import java.util.Date;
import com.cb.common.utils.DateUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import com.cb.common.utils.StringUtils;
import com.cb.common.utils.uuid.IdUtils;
import com.cb.common.utils.SecurityUtils;
import com.cb.probity.mapper.BizProbityChildSpouseMapper;
import com.cb.probity.domain.BizProbityChildSpouse;
import com.cb.probity.service.IBizProbityChildSpouseService;

/**
 * 廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况Service业务层处理
 * 
 * @author ruoyi
 * @date 2025-03-18
 */
@Service
public class BizProbityChildSpouseServiceImpl implements IBizProbityChildSpouseService 
{
    @Autowired
    private BizProbityChildSpouseMapper bizProbityChildSpouseMapper;

    /**
     * 查询廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     * 
     * @param id 廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况ID
     * @return 廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     */
    @Override
    public BizProbityChildSpouse selectBizProbityChildSpouseById(String id)
    {
        return bizProbityChildSpouseMapper.selectBizProbityChildSpouseById(id);
    }

    /**
     * 查询廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况列表
     * 
     * @param bizProbityChildSpouse 廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     * @return 廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     */
    @Override
    public List<BizProbityChildSpouse> selectBizProbityChildSpouseList(BizProbityChildSpouse bizProbityChildSpouse)
    {
        return bizProbityChildSpouseMapper.selectBizProbityChildSpouseList(bizProbityChildSpouse);
    }

    /**
     * 新增廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     * 
     * @param bizProbityChildSpouse 廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     * @return 结果
     */
    @Override
    public int insertBizProbityChildSpouse(BizProbityChildSpouse bizProbityChildSpouse)
    {
        if(StringUtils.isBlank(bizProbityChildSpouse.getId())) {
            bizProbityChildSpouse.setId(IdUtils.randomUUID());
        }
        try{
            bizProbityChildSpouse.setCreateBy(SecurityUtils.getUsername());
        } catch (Exception e){}
        bizProbityChildSpouse.setCreateTime(DateUtils.getNowDate());
        return bizProbityChildSpouseMapper.insertBizProbityChildSpouse(bizProbityChildSpouse);
    }


    /**
     * 批量新增廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     *
     * @param entities
     * @return 结果
     */
    @Override
    public int insertBatch(List<BizProbityChildSpouse> entities) {
        if(null == entities || entities.isEmpty()){
            return 0;
        }
        String userName = null;
        try{
            userName = SecurityUtils.getUsername();
        } catch (Exception e){}
        Date nowDate = DateUtils.getNowDate();
        String finalUserName = userName;
        entities.parallelStream().forEach(item -> {
            item.setId(IdUtils.randomUUID());
            item.setCreateBy(finalUserName);
            item.setCreateTime(nowDate);
        });
        // 定义每批次的大小
        int batchSize = 500;
        int totalInserted = 0;
        int num = entities.size() / batchSize + (entities.size() % batchSize == 0 ? 0 : 1);
        for (int i = 0; i < num; i++) {
            int start = i * batchSize;
            int end = Math.min(start + batchSize, entities.size());
            totalInserted +=  bizProbityChildSpouseMapper.insertBatch(entities.subList(start, end));
        }
        return totalInserted;
    }


    /**
     * 修改廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     * 
     * @param bizProbityChildSpouse 廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     * @return 结果
     */
    @Override
    public int updateBizProbityChildSpouse(BizProbityChildSpouse bizProbityChildSpouse)
    {
        try{
            bizProbityChildSpouse.setUpdateBy(SecurityUtils.getUsername());
        } catch (Exception e){}
        bizProbityChildSpouse.setUpdateTime(DateUtils.getNowDate());
        return bizProbityChildSpouseMapper.updateBizProbityChildSpouse(bizProbityChildSpouse);
    }

    /**
     * 批量删除廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况
     * 
     * @param ids 需要删除的廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况ID
     * @return 结果
     */
    @Override
    public int deleteBizProbityChildSpouseByIds(String[] ids)
    {
        return bizProbityChildSpouseMapper.deleteBizProbityChildSpouseByIds(ids);
    }

    /**
     * 删除廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况信息
     * 
     * @param id 廉政档案-7.本人子女与外国人、无国籍人及港澳、台湾居民通婚情况ID
     * @return 结果
     */
    @Override
    public int deleteBizProbityChildSpouseById(String id)
    {
        return bizProbityChildSpouseMapper.deleteBizProbityChildSpouseById(id);
    }
}
