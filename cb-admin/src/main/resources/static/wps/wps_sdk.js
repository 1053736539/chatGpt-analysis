(function (global, factory) {
    ...
    function EnableMultiUser(callback) {
        serverId = getServerId();
        InitSdk(callback, true)
    }
    ...
    function RegWebNotify(clientType, name, callBack, wpsclient) {
        ...
        var askItem = function () {
            ...
            xhr.ontimeout = function (e) {
                //国产机bug 不会走入WPSInnerMessage_quit，以clientId区分。 2023-11-14 Lzx
                if (xhr.responseText == "WPSInnerMessage_quit" || (wpsclient && (wpsclient.clientId != paramStr.clientid))){
                    if ((!wpsclient || wpsclient.single)) {
                        window.setTimeout(askItem, 2000);
                    }
                    return;
    ...
    function InitSdk(callback, bMultiUser) {
        if (bMultiUser === undefined) {
            bMultiUser = false; // 默认为单用户模式
        }
        g_isSdkInited = false;
    ...
    return {
        ...
        WpsInvoke: WpsInvoke,
        WpsClient: WpsClient,
        WebNotifyUseTimeout: WebNotifyUseTimeout,
 ...
 *
 * 改动时间：2023-02-17 18:01
 * 改动说明：1、sdk内部不执行InitSdk，抛出InitWpsSdk给用户使用，用户可以调用InitWpsSdk(callback)，
 *             在callback回调中处理业务逻辑，或者直接按原来逻辑处理，在调业务接口时会自动唤起wps
 *          2、WpsStartWrapExInner方法内默认超时时间设置为60秒
 * 改动影响：项目加载时不再多次唤起wps端口弹窗
 *
 * 改动时间：2023-11-14 16:20
 * 改动说明：1、修改RegWebNotify方法，修复国产机bug ，多进程时，Linux不会返回WPSInnerMessage_quit，以clientId区分释放轮询监听。
 *
 * 改动时间：2023-12-7 9:01
 * 改动说明：1、修改了ws、http互相替换链接的方法 transferWebsocketToHttp 和 transferHttpToWebsocket ，增加了https 和 wss 相互替换的方法，解决业务系统是https协议，无法唤起wps客户端，
 * 控制台提示Failed to construct'WebSocket':The URL's scheme must be either 'ws' or 'wss' ,'https' is not allowed.
 *
 */
