...
function OnInsertPicToDoc(doc, picPath, picWidth, picHeight) {
    ...
    var pagecount = doc.BuiltInDocumentProperties.Item(WPS_Enum.wdPropertyPages); //获取文档页数
    selection.GoTo(WPS_Enum.wdGoToPage, WPS_Enum.wdGoToPage, pagecount.Value); //将光标指向文档最后一页
    ...
    seal_shape.RelativeHorizontalPosition = WPS_Enum.wdRelativeHorizontalPositionPage;
    seal_shape.RelativeVerticalPosition = WPS_Enum.wdRelativeHorizontalPositionPage;
...
function OnInsertRedHeaderClick() {
    ...
    if (l_BkFile == "" || l_insertFileUrl == "") {
		...
		wps.PluginStorage.setItem('redHeadsPath', redHeadsPath);
		var getRedHeadPath = GetDocParamsValue(l_Doc, 'getRedHeadPath');
		wps.PluginStorage.setItem('getRedHeadPath', getRedHeadPath);
        var height = 250;
        var width = 400;
...
var msgMap = {
  401: '没有操作权限，请从考核系统重新打开文档。',
  404: '文件不存在，请从考核系统重新打开文档。',
  500: '文件保存出错，请重试或者联系管理员。',
};
/**
 * 调用文件上传到OA服务端时，
 * @param {*} resp
